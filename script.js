...
function changeLanguage(language) {
    ...
    if (courseLabel) {
        courseLabel.textContent = translations[language].courseLabel;
    }

    if (courseTitle) {
        courseTitle.textContent = translations[language].courseTitle;
    }

    if (courseText) {
        courseText.textContent = translations[language].courseText;
    }

    if (courseButton) {
        courseButton.textContent = translations[language].courseButton;
    }

    if (courseTags[0]) {
        courseTags[0].textContent = translations[language].courseLevel;
    }

    if (courseTags[1]) {
        courseTags[1].textContent = translations[language].courseCertificate;
    }

    if (languageBtn) {
        languageBtn.textContent = language === "ru" ? "EN" : "RU";
    }
...
document.querySelectorAll("form").forEach(form => {
    if (
        !form.classList.contains("feedback-form") &&
        ...
        form.addEventListener("submit", event => {
            ...
            alert(translations[currentLanguage].formSend);
        });
    }
});
